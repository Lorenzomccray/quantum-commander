...
import { runPremiumInterface } from './PremiumInterface.js';
...
export default class QuantumCommander {
  ...
  setupCommands() {
    ...
    this.program.command('premium')
      .description('Launch the premium interactive interface')
      .action(async () => {
        try {
          await runPremiumInterface();
        } catch (e) {
          console.error(chalk.red('💥 Premium interface failed:'), e.message);
        }
      });
